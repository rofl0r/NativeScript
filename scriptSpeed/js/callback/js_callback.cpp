...
namespace js {
	namespace callback {
		...
		typedef void(*jsCb)(FunctionCallbackInfo<Value> const&);

		void cb0(const FunctionCallbackInfo<Value>& args) { ::callback::processCallback(1); }
		void cb1(const FunctionCallbackInfo<Value>& args) {	::callback::processCallback(
			Local<Value>::Cast(args[0])->NumberValue());}
		void cb2(const FunctionCallbackInfo<Value>& args) {	::callback::processCallback(
			Local<Value>::Cast(args[0])->NumberValue()+Local<Value>::Cast(args[1])->NumberValue());}
		void cb3(const FunctionCallbackInfo<Value>& args) {	::callback::processCallback(
			Local<Value>::Cast(args[0])->NumberValue()+Local<Value>::Cast(args[1])->NumberValue()
			+Local<Value>::Cast(args[2])->NumberValue());}
		void cb4(const FunctionCallbackInfo<Value>& args) {	::callback::processCallback(
			Local<Value>::Cast(args[0])->NumberValue()+Local<Value>::Cast(args[1])->NumberValue()
